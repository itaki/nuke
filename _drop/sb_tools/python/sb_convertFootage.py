...
class sb_convertFootage_Panel(nukescripts.PythonPanel):
	...
	def __init__(self):
		...
		self.div12 = nuke.Text_Knob("divider12", "")
		self.render = nuke.PyScript_Knob("render", '<b><font color="green"><size><font size="6">render</font size></font></b>')
	...
	def convertFootage(self):
		...
		# Framepadding is a bitch...
		# If a file is named myFile.0021.exr we assume framepadding.
		# In other words, if the separator is ".", we assume it's framepadding.
		# But if a filename has a separator of "_" or "-", we have no way to know if the numbers are framedadding or part of the name.
		# An example of this is images from a Canon DSLR: IMG_2312.JPG.

		# So, let's put some hacky stuff together.
		# Loop through all selected Read nodes and extract components from it's name.
		# Store info in per node in a dict, and then store all the dicts in a list.

		# Separators that may or may not be followed by frame-padding.
		separatorPadding = {"_":"", "-":""}

		readNodesErrorChecked = []
		for i in reads:
			...
			checkedRead = {}
			checkedRead["node"] = i
			checkedRead["folderPath"] = origFolderPath
			checkedRead["fileName"] = fileName

			# If a separator is found...
			if separator:
			# Some files with suspicios name found...
				if separator in ["_", "-"]:

					# First, let's check if the separator is set in the separatorPadding dict. If not, we need to bring up the dialog.
					if separatorPadding[separator] == "":
						p = nuke.Panel("Is there framepadding in: {0}?".format(origFullFileName))
						p.addBooleanCheckBox('Assume the same for all similar filenames.', False)
						p.addButton("Cancel")
						p.addButton("Yes")
						p.addButton("No")
						p.setWidth(600)
						result = p.show()
						assume = p.value('Assume the same for all similar filenames.')

						# If framepadding.
						if result == 1:
							if assume:
								separatorPadding[separator] = True

						elif result == 2:
							checkedRead["hasFramePadding"] = False
							fileName = "{0}{1}{2}".format(fileName, separator, framePadding)
							checkedRead["fileName"] = fileName
							if assume:
								separatorPadding[separator] = False
							# Add item to dict and continue with the next item...
							readNodesErrorChecked.append(checkedRead)
							continue
						# If cancel.
						else:
							return

					# If the separator in the separatorPadding dict is set to False, it means no frame padding.					
					elif separatorPadding[separator] == False:
							checkedRead["hasFramePadding"] = False
							fileName = "{0}{1}{2}".format(fileName, separator, framePadding)
							checkedRead["fileName"] = fileName
							# Add item to dict and continue with the next item...
				...
				#If we end up down here, it probably (hopefully) means that we have framepadding.
				# We should end up here if the separator is "." or if the separatorPadding dict is set to True.
				checkedRead["hasFramePadding"] = True
				checkedRead["separator"] = separator
				checkedRead["framePadding"] = framePadding
				readNodesErrorChecked.append(checkedRead)

		# Number in filename when using counter.
		currNum = 1

		# Loop over read nodes create render setups.
		# We needed to do error checking prior to looping (otherwise you couldn't fire off a render and leave, as a dialog might pop-up asking if a file has framepadding or not).
		# All info is stored in a dict.
		# So let's loop through the dict instead of the selected nodes directly.
		# Looks a bit hacky and potentially slow, but it should work in more cases this way.
		for i in readNodesErrorChecked:

			# Get values from dict.
			readNode = i["node"]
			fileName = i["fileName"]
			origFolderPath = i["folderPath"]
			hasFramePadding = i["hasFramePadding"]
			try:
				separator = i["separator"]
				framePadding = i["framePadding"]
			except:
				pass
			...
			# Setup/save values.
			formatName = readNode["format"].value()
			readLabel = readNode["label"].value()
			rawVal = readNode["raw"].value()

			# Framerange.
			if frameRange == "current range":
				ff = readNode["first"].value()
				lf = readNode["last"].value()

			# If using a global range, there's no need to render more than one frame if the original is one frame only...
			if readNode["first"].value() == readNode["last"].value():
				lf = ff

			if fileNameMethod == "custom filename":
				readNode["label"].setValue(customFileName)
				fileName = readNode["label"].evaluate()

			# Replace word.
			if replace and replaceSrc:
				readNode["label"].setValue(replaceSrc)
				replaceSrc = readNode["label"].evaluate()
				readNode["label"].setValue(replaceDst)
				replaceDst = readNode["label"].evaluate()
			...
			# Add prefix/suffix.
			if prefix:
				readNode["label"].setValue(prefix)
				fileName = "{0}{1}".format(readNode["label"].evaluate(), fileName)

			if suffix:
				readNode["label"].setValue(suffix)
				fileName = "{0}{1}".format(fileName, readNode["label"].evaluate())
			...
			if rootFolderMethod == "custom path":
				readNode["label"].setValue(customRootFolder)
				folderPath = readNode["label"].evaluate()
			else:
			...
			if i["hasFramePadding"]:
				...
				if ff == lf:
					...
					paddingDef = 4
					currDigitLen = len(str(readNode["last"].value()))
			...
			if renderAction == "print render path (debugging)":
				renderPathMsg = "{0} ({1}): {2}".format(sb_convertFootage_Data()["scriptName"], readNode["name"].value(), renderPath)
				print renderPathMsg
				continue
			...
			# If using no conversion mode.
				...
				colorOut = readNode["colorspace"].value()
				readNode["raw"].setValue(True)
			...
			readNode["label"].setValue(readLabel)

			# Refocus on main tab of Read node.
			readNode["file"].setFlag(0)
			...
			if renderAction == "render selected":
				...
				t = sb_convertFootage_renderWrites(w, ff, lf, readNode, colorSpaceOut, colorSpaceOutOCIO, rawVal, formatName, createdNodes, postRenderAction)
				t.start()

			elif renderAction == "setup nodes (no render)":
				print "{0} created a render setup for {1}. Render path set to: {2}.".format(sb_convertFootage_Data()["scriptName"], readNode["name"].value(), renderPath)

		return True

# Run main script.
def sb_convertFootage(showAsModal=True):
