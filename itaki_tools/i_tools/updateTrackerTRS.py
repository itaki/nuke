...
def update_tracker_TRS(trans, rot, scale, all=False):
	...
	if node.Class() == 'Tracker4':
		...
		if (trans == True and rot == False and scale == False and all==False):
		...
		elif (trans == False and rot == True and scale == False and all==False):
			for i in range(0, num_tracks):
		...
		elif (trans == False and rot == False and scale == True and all==False):
			for i in range(0, num_tracks):
				...
				if track_list.getValue(31*i+8) == 0:
					...
					track_list.setValue(scale, 31*i+8)
		# validate All
		elif (trans == False and rot == False and scale == False and all==True):
			for i in range(0, num_tracks):
				# check to see if S is checked or not
				if track_list.getValue(31*i+8) == 0:
					# set S to True
					track_list.setValue(1, 31*i+8)
				else: 
					# set S to False
					
					track_list.setValue(0, 31*i+8)
			for i in range(0, num_tracks):
				# check to see if R is checked or not
				if track_list.getValue(31*i+7) == 0:
					# set R to True
					track_list.setValue(1, 31*i+7)
				else: 
					# set R to False
					
					track_list.setValue(0, 31*i+7)
			for i in range(0, num_tracks):
				# check to see if T is checked or not
				if track_list.getValue(31*i+6) == 0:
					# set T to True
					track_list.setValue(1, 31*i+6)
				else: 
					# set T to False
					
					track_list.setValue(0, 31*i+6)
		else:
			pass
