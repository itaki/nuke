...
TIMER = 60 # this uprdates every x seconds
IDLE_TIME = 300 # starts idling afert x seconds
TESTING = False
USE_VIEWER = True
GAME = True
...
ACHIEVEMENT_SCORE = [1,2,5,10,15,30,60,120,240,480,600,720,960,1440,2880,5760,7200,10080,13440,20160,40320]
if THEME == 'animals':
    ACHIEVEMENTS = ['🐣','🐥','🐸','🐷','🐮','🐹','🐰','🐶','🐨','🐵','🐼','🐻','🦊','🐙','💀','👹','👿','👽','🦁','🐯','🐲']
elif THEME == 'objects':
    ACHIEVEMENTS = ['🍼','🎈','🔫','🏓','🌮','🌈','🍯','🌊','🌋','🍺','🍩','🎩','🍷','☕','☔','🦄','🎥','🦖','🧘','🧟','👑']
...
class Timelogger():
    ...
    def start_session(self):
        ...
        it figures out path stuff
        gets the log if there is one, but creates one if not
        creates a new session in the log
    ...
    def create_new_session(self):
        ...
        self.log_data['header']['number_of_sessions'] = len(self.log_data['sessions'])
        self.save_log()
    ...
    def update_session(self): 
        ...
        # Then send it onnn
        self.log_data['sessions'][i]['end_time'] = self.now()
        if self.recently_saved == True:
            self.log_data['sessions'][i]['status'] = 'saved'
            self.recently_saved = False
        else:
            self.log_data['sessions'][i]['status'] = 'unsaved'

        if self.calculate_idle_time() > IDLE_TIME:
            self.log_data['sessions'][i]['idle_time'] += TIMER
            self.idling = True
        else:
            self.idling = False
    ...
    def calculate_stats(self):
        ...
        print(f"mnumber of sessions {self.number_of_sessions}")


    def get_session(self):
        for i in range(len(self.log_data['sessions'])):
            if self.log_data['sessions'][i]['start_time'] == self.id:
                return i
            
    def save_session(self):
        if self.log_data == None:
            self.start_session()
        # MAKE it create a log first
        self.recently_saved = True
        self.update_session()
    ...
    def select_achievement(self, all_work):
        if ACHIEVEMENTS != int: # if achie
            for i in range(len(ACHIEVEMENT_SCORE)):
                if all_work > ACHIEVEMENT_SCORE[i]:
